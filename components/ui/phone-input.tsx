...
export const PhoneInput = forwardRef<HTMLInputElement, PhoneInputProps>(({ className, onCountryChange, onChange, value, placeholder, defaultCountry, inline = false, ...props }, ref) => {
  ...
  return (
    <div className={inputClasses}>
      {!inline && <div className="w-4 h-4 rounded-full shrink-0">{displayFlag ? <CircleFlag countryCode={displayFlag} height={16} /> : <GlobeIcon size={16} />}</div>}
      <input
        ref={ref}
        value={value}
        ...
        autoComplete="tel"
        name="phone"
        className={cn("flex w-full border-none bg-transparent text-base transition-colors placeholder:text-muted-foreground outline-none h-9 py-1 p-0 leading-none md:text-sm [interpolate-size:allow-keywords]", className)}
        {...props}
      />
    </div>
  );
